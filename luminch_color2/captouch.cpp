...
#define CAP_BUF_LENGTH (53 * CAP_N_SENSORS + 2)
...
void captouch_get_readings(void)
{
  ...
  for (index = 0; index < CAP_N_SENSORS * CAP_OVERSAMPLING; index++) {
    ...
    // read sensor
    sample1 = analogRead(ch_pin);
    
    
    // Do the same but with inverse polarity
    // set reference pin as output and low
    pinMode(A0 + ref_pin, OUTPUT);
    digitalWrite(A0 + ref_pin, LOW);
	
    // set sensor pin as output and high (charge sensor)
    pinMode(A0 + ch_pin, OUTPUT);
    ...
    for (i = 0; i < CAP_ANALOG_INTEGRATE - 1; i++) {
      // set ADC to reference pin (discharge internal capacitor)
      captouch_set_adc_reference_pin(ref_pin);
      // set ADC to sensor pin (transfer charge from Csense to Chold)
      captouch_set_adc_reference_pin(ch_pin);
    }
    ...
    // read sensor
    sample2 = analogRead(ch_pin);
    
    #ifndef CS_ENABLE_SLEW_RATE_LIMITER
    captouch.readings[ch] += 32768 + sample1 - sample2;
    #else
    if (captouch.readings[ch] == 0)
      captouch.readings[ch] = 32768 + sample1 - sample2;
    else
      captouch.readings[ch] += ((32768 + sample1 - sample2) > captouch.readings[ch]) ? 1: -1;
    #endif  
