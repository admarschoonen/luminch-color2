...
#define HIGH_POWER_PIN  5

#define NUM_LEDS_INNER  16
#define NUM_LEDS_OUTER  24
#define NUM_LEDS        (NUM_LEDS_INNER + NUM_LEDS_OUTER)
#define NUM_LEDS_SHOW   NUM_LEDS
...
void setup()
{
  captouch_init();
  Serial.begin (115200);
  Serial.println("Luminch Color V2!");
...
void process_ds_distance(cs_Lab_t *Lab, cs_Lab_t *Lab_target, char *power) {
  ...
  if (ds.distance <= DS_FARTHEST) {
    ...
    if (ds_hand_present_counter > 0) {
      /* hand was present */
      if (ds_hand_present_counter <= DS_ON_OFF_MAX_LENGTH) {
	if (ds_hand_present_counter >= DS_ON_OFF_MIN_LENGTH) {
...
int led_color_interp(int pos, int c)
{
  int v;
	
  if (pos < NUM_LEDS_INNER) {
    if (pos & 0x01) {
      // inner circle, odd led
      if ((c & 0x01) && (c & 0x02))
        v = (c >> 2) + 1;
      else
        v = (c >> 2);
    } else {
      // inner circle, even led
      if (c & 0x02)
        v = (c >> 2) + 1;
      else
        v = (c >> 2);
    ...
    if (pos & 0x01) {
      // outer circle, odd led
      if ((c & 0x01) || (c & 0x02))
        v = (c >> 2) + 1;
      else
        v = (c >> 2);
    } else {
      // outer circle, even led
      v = (c >> 2);
    }
  }
...
void led_write_red(float c)
{
#if (LED_TYPE == LED_TYPE_RGB)
  analogWrite(LED_PIN(RED), (int) (255 * c));
#endif
#if (LED_TYPE == LED_TYPE_WS2812)
  int tmp = (int) (1023 * c);

  if (digitalRead(HIGH_POWER_PIN)) {  
    for (int k = 0; k < NUM_LEDS_SHOW; k++) {
      leds[k].r = led_color_interp(k, tmp);
    }
  } else {
    leds[0].r = led_color_interp(0, tmp);
    leds[NUM_LEDS_INNER - 1].r = led_color_interp(NUM_LEDS_INNER - 1, tmp);
    leds[NUM_LEDS_INNER].r = led_color_interp(NUM_LEDS_INNER, tmp);
    leds[NUM_LEDS - 1].r = led_color_interp(NUM_LEDS - 1, tmp);
  }
...
void led_write_green(float c)
{
#if (LED_TYPE == LED_TYPE_RGB)
  analogWrite(LED_PIN(GREEN), (int) (255 * c));
#endif
#if (LED_TYPE == LED_TYPE_WS2812)
  int tmp = (int) (1023 * c);
  ...
  if (digitalRead(HIGH_POWER_PIN)) {
    for (int k = 0; k < NUM_LEDS_SHOW; k++) {
      leds[k].g = led_color_interp(k, tmp);
    }
  } else {
    leds[0].g = led_color_interp(0, tmp);
    leds[NUM_LEDS_INNER - 1].g = led_color_interp(NUM_LEDS_INNER - 1, tmp);
    leds[NUM_LEDS_INNER].g = led_color_interp(NUM_LEDS_INNER, tmp);
    leds[NUM_LEDS - 1].g = led_color_interp(NUM_LEDS - 1, tmp);
  }
...
void led_write_blue(float c)
{
#if (LED_TYPE == LED_TYPE_RGB)
  analogWrite(LED_PIN(BLUE), (int) (255 * c));
#endif
#if (LED_TYPE == LED_TYPE_WS2812)
  int tmp = (int) (1023 * c);
  ...
  if (digitalRead(HIGH_POWER_PIN)) {
    for (int k = 0; k < NUM_LEDS_SHOW; k++) {
      leds[k].b = led_color_interp(k, tmp);
    }
  } else {
    leds[0].b = led_color_interp(0, tmp);
    leds[NUM_LEDS_INNER - 1].b = led_color_interp(NUM_LEDS_INNER - 1, tmp);
    leds[NUM_LEDS_INNER].b = led_color_interp(NUM_LEDS_INNER, tmp);
    leds[NUM_LEDS - 1].b = led_color_interp(NUM_LEDS - 1, tmp);    
  }
...
void led_write(cs_linear_t *cs_linear)
{
  ...
  tmp_r = (int) (1023 * cs_linear->r);
  tmp_g = (int) (1023 * cs_linear->g);
  tmp_r = (int) (1023 * cs_linear->b);
  
  memset(leds, 0, NUM_LEDS * 3);
  if (digitalRead(HIGH_POWER_PIN)) {
    for (int k = 0; k < NUM_LEDS_SHOW; k++) {
      leds[k].r = led_color_interp(k, tmp_r);
      leds[k].g = led_color_interp(k, tmp_g);
      leds[k].b = led_color_interp(k, tmp_b);
    }
  } else {
    leds[0].r = (int) (255 * (cs_linear->r));
    leds[0].g = (int) (255 * (cs_linear->g));
    leds[0].b = (int) (255 * (cs_linear->b));
    leds[NUM_LEDS_INNER - 1].r = (int) (255 * (cs_linear->r));
    leds[NUM_LEDS_INNER - 1].g = (int) (255 * (cs_linear->g));
    leds[NUM_LEDS_INNER - 1].b = (int) (255 * (cs_linear->b));
    leds[NUM_LEDS_INNER].r = (int) (255 * (cs_linear->r));
    leds[NUM_LEDS_INNER].g = (int) (255 * (cs_linear->g));
    leds[NUM_LEDS_INNER].b = (int) (255 * (cs_linear->b));
    leds[NUM_LEDS - 1].r = (int) (255 * (cs_linear->r));
    leds[NUM_LEDS - 1].g = (int) (255 * (cs_linear->g));
    leds[NUM_LEDS - 1].b = (int) (255 * (cs_linear->b));
  }
  FastLED.show();
#endif
}
